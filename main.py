...
# NTP同步（非阻塞）
def sync_time():
    global need_ntp_sync, last_ntp_sync
    try:
        ...
        rtc = RTC()
        now = rtc.datetime()
        year, month, day, weekday, hour, minute, second, subsecond = now
        hour = (hour + 8) % 24
        days_to_add = (hour + 8) // 24
...
if __name__ == '__main__':
    ...
    while (1):
        if iftimesync == True:  # 所有显示在首次ntp时间更新完成后显示，否则显示--:-- --
            ...
            # key1_mode:0：显示实时时间
            if key1_mode == 0:
                tm.write([SEG8Code[hour // 10], SEG8Code[hour % 10] | Dot, SEG8Code[minute // 10], SEG8Code[minute % 10],
                          SEG8Code[second // 10], SEG8Code[second % 10]])
                # print(year%100)
                print('当前时间：', hour, ':', minute, ':', second)

            # key1_mode:1：显示年月日
            elif key1_mode == 1:
                tm.write([SEG8Code[month // 10], SEG8Code[month % 10], SEG8Code[day // 10], SEG8Code[day % 10],
                          SEG8Code[(year % 100) // 10], SEG8Code[(year % 100) % 10]])
                print('当前日期：', year, '-', month, '-', day)

            # key1_mode:2：显示闹钟
            elif key1_mode == 2:
                if nao_zhong_zhuang_tai == 1:
                    tm.write([SEG8Code[alarm_hour // 10], SEG8Code[alarm_hour % 10] | 0x80, SEG8Code[alarm_min // 10],
                              SEG8Code[alarm_min % 10], SEG8Code[17], SEG8Code[18]])  # 尾灯显示on
                    # print(alarm_hour,alarm_min)
                else:
                    tm.write([SEG8Code[alarm_hour // 10], SEG8Code[alarm_hour % 10] | 0x80, SEG8Code[alarm_min // 10],
                              SEG8Code[alarm_min % 10], SEG8Code[17], SEG8Code[15]])  # 尾灯显示mode2
                    # print(alarm_hour,alarm_min)
                print('闹钟时间：', alarm_hour, ':', alarm_min)

            # key1_mode:3：设置闹钟hour
            elif key1_mode == 3:
                tm.write([SEG8Code[alarm_hour // 10], SEG8Code[alarm_hour % 10] | 0x80, SEG8Code[16], SEG8Code[16],
                          SEG8Code[16], SEG8Code[16]])  # 尾灯显示mode3
                # print(alarm_hour,alarm_min)
                print('设置闹钟小时：', alarm_hour)

            # key1_mode:4：设置闹钟min
            elif key1_mode == 4:
                tm.write([SEG8Code[16], SEG8Code[16] | 0x80, SEG8Code[alarm_min // 10], SEG8Code[alarm_min % 10],
                          SEG8Code[16], SEG8Code[16]])  # 尾灯显示mode4
                # print(alarm_hour,alarm_min)
                print('设置闹钟分钟：', alarm_min)

            # key1_mode:5：计时器
            elif key1_mode == 5:
                tm.write([SEG8Code[count_hour // 10], SEG8Code[count_hour % 10] | 0x80, SEG8Code[count_min // 10],
                          SEG8Code[count_min % 10], SEG8Code[count_sec // 10], SEG8Code[count_sec % 10]])
